"""
Extracts and formats column comments from ER/Studio metadata for physical models, 
cleaning any RTF/HTML formatting, and generates individual SQL comment files for each model.
"""
...
# Modified function to write column comments to a single file in the specified folder
def write_column_comments(model, column_comments):
    output_folder = os.path.join(OUTPUT_ROOT, "Get_Column_Comment_Statements_output")
    os.makedirs(output_folder, exist_ok=True)

    filename = f"{model}_column_comments.sql"
    file_path = os.path.join(output_folder, filename)

    def format_comment(comment):
        match = re.match(r"(COMMENT ON COLUMN .+? IS )'(.+)';", comment)
        if match:
    ...
    if column_comments:
        with open(file_path, 'w', encoding='utf-8') as file:
            ...
            file.write(f"-- Column Comments for {model}\n\n")
        ...
        logging.info(f"Column comments for {model} written to {file_path}")
    else:
        ...
        for model in physical_models:
            try:
                ...
                if er_studio_metadata:
                    ...
                    # Write column comments to a file
                    write_column_comments(model, cleaned_metadata)
